...
'''
class VelocityMatch: 
    def __init__(self, max_linear_acceleration=5, time_to_target=0.01):
        self.max_linear_acceleration = max_linear_acceleration
    ...
    def getSteering(self, character, target):
        ...
        steering.linear = Vec2D.sub(target.linear_velocity, character.linear_velocity)
        steering.linear.div(self.time_to_target)

        if steering.linear.length() > self.max_linear_acceleration:
            steering.linear.change_length(self.max_linear_acceleration)
...
class Pursue: 
    def __init__(self, max_prediction=2):
    ...
    def getSteering(self, character, target):
        ...
        current_speed = character.linear_velocity.length()

        # Check if speed is twoo small to give a reasonable prediction time
        if current_speed <= distance / self.max_prediction:
        ...
        new_target.position = new_target.position.add(new_target.linear_velocity.mul(prediction))
        return seek.getSteering(character, new_target)

class Align: 
    def __init__(self, max_angular_acceleration=0.1*math.pi, max_angular_speed=0.05, target_radius=0.01*math.pi, slow_radius=0.5*math.pi, time_to_target=0.1):
...
class Wander: 
    def __init__(self, wander_offset=50, wander_radius=50, wander_rate=0.1*math.pi, wander_orientation=1, max_linear_acceleration=1):
